...
_Selene is free software, MIT licensed, sponsored by [SeleneDAO](https://www.SeleneDAO.com)_

## What is Selene 🐐?
Selene 🐐 (Great Onchain Agent Toolkit) is an open-source framework for adding blockchain tools such as wallets, being able to hold or trade tokens, or interacting with blockchain smart contracts, to your AI agent.
...
2. **Wallet Agnostic**: Supports all wallets, from your own key pairs to [SeleneDAO Smart Wallets](https://docs.SeleneDAO.com/wallets/smart-wallets/overview) and Coinbase.
...
### How it works
Selene plugs into your agents tool calling capabilities adding all the functions your agent needs to interact with blockchain protocols. 

High-level, here's how it works:

<Steps>
  <Step title="Configure the wallet you want to use">
  Each wallet will have their specific client, e.g `viem(wallet)` for EVM key pairs or `smartwallet(wallet)` for SeleneDAO smart wallets.
```typescript
const wallet = ...
...
const result = await generateText({
    ...
    tools,
    maxSteps: 5,
    prompt: "Send 420 ETH to ohmySelene.eth",
});
```
  </Step>
</Steps>
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    href="/wallets"
  >
    Use any wallet you want, from key pairs to smart wallets from SeleneDAO
  </Card>
  <Card
    title="Bounties"
    icon="coin"
